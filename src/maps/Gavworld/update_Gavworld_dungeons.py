#!/usr/bin/env python3
"""
Sync individual dungeon map JSON files from Gavworld_dungeons.json.

Gavworld_dungeons.json is edited in Tiled and may reference any of:
...
three-tileset layout that the engine consumes:
  - Gavworld_common  (inherited — written by update_Gavworld.py)
  - Gavworld_outside (inherited — written by update_Gavworld.py)
  - Gavworld_dungeons (owned — written here)
...
the lookup warns and the tile is skipped; re-run update_Gavworld.py first.

caves tiles are assigned compact map GIDs in a fresh Gavworld_dungeons
...
def make_dungeons_tilesets(common_count, outside_count):
    ...
    return [
        ...
        {'firstgid': dungeons_firstgid, 'source': '../../tileset/maps/Gavworld_dungeons.json'},
    ]
...
def main():
    master, master_tilelayers, master_objs, maps_layer = lib.load_master(
        lib.MAPS_DIR / 'Gavworld_dungeons.json'
    )
    if not maps_layer:
        print('ERROR: no "maps" objectgroup found in Gavworld_dungeons.json')
        return
    ...
    if not catalogue:
        print('ERROR: Gavworld_dungeons.json has no recognised tilesets')
        return
    ...
    print(f'Compact GID map: {len(src_to_map_gid)} src tiles '
          f'({dungeons_count} caves, '
          f'{len(src_to_map_gid)-dungeons_count} inherited from Gavworld_common/outside)')

    dungeons_ts_path = lib.TILESET_DIR / 'maps' / 'Gavworld_dungeons.json'
    dungeons_ts_json = lib.load_or_init_tileset(
        dungeons_ts_path, 'Gavworld_dungeons', 'Gavworld_dungeons.png',
    ...
    for zone in lib.iter_zones(maps_layer, name_to_file_overrides=NAME_TO_FILE,
        ...
        print('  syncing tile layers from master Gavworld_dungeons.json')
