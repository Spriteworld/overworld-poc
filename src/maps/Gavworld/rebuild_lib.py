...
Each `update_Gavworld*.py` script reads a Tiled master map, derives per-zone
...
(Gavworld.json / Gavworld_inside.json / Gavworld_dungeons.json) and lives here.
...
What stays per-script:
  * The GID-assignment policy. The three masters share their split rules
    ...
    common+outside and adds caves). Each script defines its own
...
def ensure_scene_file(scene_key, *, inside=False, world_prefix=None):
    """Create `src/scenes/maps/Gavworld/{scene_key}.js` if missing. Inside
    ...
    """
    new_path = SRC_DIR / 'scenes' / 'maps' / 'Gavworld' / f'{scene_key}.js'
    if new_path.exists():
        return False
    ...
    if world_prefix and scene_key.startswith(world_prefix) and scene_key != world_prefix:
        ...
        legacy_path      = SRC_DIR / 'scenes' / 'maps' / 'Gavworld' / f'{legacy_scene_key}.js'
...
def ensure_maps_index(scene_key, fname, inside=False, *, world_prefix=None):
    ...
    Migration: any existing `import OLDMap from './Gavworld/{fname}'` whose var
    ...
    `Gavworld.world`, and GavworldWorld's merge step strips warps targeting
    anything in it.
    """
    ...
    m = re.search(
        r"import (\w+) from " + re.escape(f"'./Gavworld/{fname}'"), content,
    )
    if m and m.group(1) != map_var:
        legacy_var = m.group(1)
        # Rename the import line
        content = content.replace(
            f"import {legacy_var} from './Gavworld/{fname}'",
            f"import {map_var} from './Gavworld/{fname}'",
        )
    ...
    # appear here; GavworldWorld._buildWorldTilemap drops every warp whose
...
def ensure_scenes_index(scene_key, *, world_prefix=None):
    ...
    matching `import {legacy} from '@Scenes/maps/Gavworld/{legacy}.js';`
    ...
    if world_prefix and scene_key.startswith(world_prefix) and scene_key != world_prefix:
        ...
        legacy_import    = (
            ...
            f"'@Scenes/maps/Gavworld/{legacy_scene_key}.js';"
        )
        if legacy_import in content:
            new_import = (
                f"import {scene_key} from "
                f"'@Scenes/maps/Gavworld/{scene_key}.js';"
            )
    ...
    if f"import {scene_key} from" not in content:
        content, ok = js_insert_after_last(
            content,
            r"import \w+ from '@Scenes/maps/Gavworld/[^']+\.js';",
            f"import {scene_key} from '@Scenes/maps/Gavworld/{scene_key}.js';"
        )
        if ok:
            changed = True
...
def sweep_legacy_world_namespace(world_prefix):
    ...
    This sweep finishes the rename: every `import \\w+Map from './Gavworld/...';`
    ...
    value. Same for src/scenes/index.js + src/scenes/maps/Gavworld/*.js scene
    ...
    for m in list(re.finditer(
        r"import (\w+)Map from '\./Gavworld/([\w./]+\.json)';", content,
    )):
        old_stem  = m.group(1)
        old_fname = m.group(2)
        ...
        content = content.replace(
            f"import {old_var} from './Gavworld/{old_fname}'",
            f"import {new_var} from './Gavworld/{old_fname}'",
        )
        content = re.sub(
    ...
    for m in list(re.finditer(
        r"import (\w+) from '@Scenes/maps/Gavworld/(\w+)\.js';", content,
    )):
        var_name   = m.group(1)
        ...
        content = content.replace(
            f"import {var_name} from '@Scenes/maps/Gavworld/{var_name}.js';",
            f"import {new_name} from '@Scenes/maps/Gavworld/{new_name}.js';",
        )
        content = re.sub(
    ...
    # ── src/scenes/maps/Gavworld/*.js files ───────────────────────────────
    scenes_dir = SRC_DIR / 'scenes' / 'maps' / 'Gavworld'
