...
─────────────────────
1. Reads Gavworld_inside.json's "maps" objectgroup to derive each named area's
   tile bounds.
2. For each map file:
   ...
   - Extracts tile data from Gavworld_inside.json, remapping combined source GIDs
     to Gavworld_inside GIDs via inside_gid_map.json.
   - For any source GID with no Gavworld_inside equivalent (new tiles added in
     Tiled), appends new entries to the Gavworld_inside tileset JSON and updates
     inside_gid_map.json so subsequent runs keep the assignment stable.
   - Merges interaction objects (matched by name) from Gavworld_inside.json's
...
# Maps a Gavworld_inside.json map name to the JS scene key that should be used
# when registering it.  Use this when the map already exists under a
# different name registered by update_Gavworld.py (e.g. "ProfLab" ↔ "ProfessorLab").
NAME_TO_SCENE_KEY = {
...
# gen3_outside tile IDs (0-based) that must always live in Gavworld_common so
...
def make_inside_tilesets(common_count):
    return [
        ...
        {'firstgid': common_count + 1, 'source': '../../tileset/maps/Gavworld_inside.json'},
    ]
...
def main():
    master, master_tilelayers, master_objs, maps_layer = lib.load_master(
        lib.MAPS_DIR / 'Gavworld_inside.json'
    )
    if not maps_layer:
    ...
    for zone in lib.iter_zones(maps_layer, name_to_file_overrides=NAME_TO_FILE,
        ...
        print('  syncing tile layers from master Gavworld_inside.json')
