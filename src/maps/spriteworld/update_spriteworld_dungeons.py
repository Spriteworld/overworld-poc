...
spriteworld_dungeons.json is edited in Tiled and may reference any of:
...
the lookup warns and the tile is skipped; re-run update_spriteworld.py first.

cave_dungeon tiles are assigned compact map GIDs in a fresh spriteworld_dungeons
...
def build_compact_gid_map(master_tilelayers, catalogue,
                            gen3_raw_to_spriteworld, common_count, outside_count):
    """
    Build src_gid → map_gid mapping for dungeon maps.
    - gen3_outside tiles → spriteworld_common / spriteworld_outside via gid_map.json
    ...
    update_spriteworld.py to surface a missing outdoor tile.
    """
    DUNGEONS_FIRSTGID = common_count + outside_count + 1
        ...
        if name == 'gen3_outside':
            ...
            if kgid is None:
                print(f'  WARNING: gen3_outside tile {tid} (src_gid={src_gid}) '
                      f'not in gid_map.json — skipping. Re-run update_spriteworld.py first.')
                continue
    ...
    # Assign cave_dungeon tile ids in numerical order so the spriteworld_dungeons
...
def update_dungeons_png(src_to_map_gid, common_count, outside_count,
    ...
    """Rebuild spriteworld_dungeons.png from cave_dungeon tiles only."""
    DUNGEONS_FIRSTGID = common_count + outside_count + 1
...
def main():
    ...
    outside_count     = len(gen3_raw_to_spriteworld) - common_count
